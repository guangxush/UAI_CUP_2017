...
def generate_train_input(input_file, output_file):
    ...
    df_sort = df.sort_values(by=['start_geo_id', 'end_geo_id', 'create_hour', 'create_date'])
    df_sort['create_hour'] = df_sort['create_hour'].astype('int')
    df_sort['order_count'] = df_sort['order_count'].astype('int')
    df_sort.to_csv(out_file, index=False, header=True, sep=',')
